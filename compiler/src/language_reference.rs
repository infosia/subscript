...
const Q34_ASYNC: &str = "Q34/R13/R36/§70 admits `await Context.suspend()`, direct awaits of named async functions and async instance methods, and a held async handle awaited later. The named async function and reference class can be generic. Direct calls accept `await f(...)`, `await f<A>(...)`, `await recv.m(...)`, and `await recv.m<A>(...)` (§93). `Promise<T>` is the TypeScript-compatible annotation and storage view of that handle, not a Promise object: constructors, `then`, statics, and combinators do not exist. Handles may be stored in locals and arrays or passed to another function, but every created handle must have at least one awaited completion. Copies retain the Context-owned frame, lexical exits release it, and `await` does not change ownership. Under \u{a7}94 an async call runs its callee to that callee's first await or return, and every await suspends its caller, a completed handle included: the await registers a continuation and returns. A completion queues the continuations registered on it, in registration order. Only a host checkpoint runs those continuations, so there is no event loop and no autonomous scheduler.";
...
const FEATURES: &[Feature] = &[
    ...
    Feature {
        ...
        corpus: &[
            ...
            "corpus/accept/a179-static-read-accessor.ts",
    ...
    Feature {
        ...
        corpus: &[
            ...
            "corpus/reject/r186-async-generic-method-without-type-args.ts",
            "corpus/reject/r187-async-generic-method-on-value-class.ts",
        ],
    },
    Feature {
