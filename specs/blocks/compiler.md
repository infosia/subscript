...
| §2 | Oracle: golden corpus outputs | active |
| §3 | Pre-registered criteria | active |
| §4 | Milestones and gates | active |
| §5 | Conventions | active |
| §6 | P1 checker contract | active |
| §7 | P2 runtime and JIT contract | active |
| §8 | P3 AOT, hot reload, and the standing gate | active |
| §9 | P4 measurement methodology | active |
| §10 | P4.1 lowering optimization and re-measurement | active |
| §10a | Emitted-C growable-array element access is inlined | active |
| §11 | P4.3 ship tier — C emission (LLVM) | active |
...
| §14 | P7 — async/Future model and remaining production shapes | active |
| §15 | P9 — standard library (`Math`, `Date`) | active |
| §16 | P14 — narrow numerics (`i8`/`u8`/`i16`/`u16`/`f16`) | active |
| §17 | P16 — generated API reference | active |
| §18 | The host Context C API, and the trap observer | active; 18.2e is history (superseded by §21.2) |
| §19 | P19 — trap unwind parity (CRITICAL) — RESOLVED 2026-07-26 | history — `compiler-history.md` §19 (resolved 2026-07-26) |
| §20 | P20 — the trap-site IR | active |
| §21 | P21 — the allocation path: fault injection and per-allocation attribution | active |
| §22 | P24 — two monotonic costs under invariant 2 | active |
| §23 | P25 — no header is privileged | active |
| §24 | Q32 — string-literal union aliases | active |
| §25 | Q33 — literal-constructible descriptor classes | active |
| §26 | Q34 — async/await, poll-driven | active |
| §27 | R5 — scalar array-pairs at parameter position | active |
| §28 | R6 — string-view fields in boundary structs | active |
| §29 | AI-facing generated reference | active |
| §30 | R7 — nested aggregates beside string fields; struct-level scalar/enum pairs | active |
| §31 | R8 — opaque handles in aggregate positions | active |
| §32 | R9 — recursive lowering at embedded positions | active |
| §33 | R10 — lowering through struct-pointer members | active |
| §34 | R11 — parameter-position handle-element pairs | active |
| §35 | R12 — `_Nullable` handle parameters | active |
| §36 | OBS-1 — emitted C names every referenced boundary typedef | active |
| §37 | R13 — async instance methods on reference classes | active |
| §38 | Workers round 1 — module state is Context state in both tiers | active |
| §39 | Workers round 2 — runtime threads, channels, worker lifecycle | active |
| §40 | Workers round 3 — the language surface (Q35) | active |
| §41 | R14 — `switch` over Q32 literal-union aliases | active |
| §42 | R15 — divergence flow: exhaustive switches and `unreachable()` | active |
| §43 | R16 — absence-capable Q32-alias descriptor members | active |
| §44 | OBS-3 — scalar handle fields beside arrays in scratch-lowered structs | active |
| §45 | R18 — contextual typing for conditional expressions | active |
| §46 | R19 — narrowing flows into conditional arms | active |
| §47 | OBS-4 — AAPCS64 packs eightbytes, not fields (CRITICAL) | active |
| §48 | R20 — external types in a generated mirror | active |
| §49 | R21 — a host-driven ship-tier form | active |
| §50 | R23 — wire-mapped literal-union aliases (`CEnum`) | active |
| §51 | R24 — `subscript bind` emits CEnum references (`@subscript-cenum`) | active |
| §52 | Wire-mapped aliases in boundary structs | active |
| §53 | R25 — entry-less dev sessions | active |
| §54 | Caller link inputs follow the translation units | active |
| §55 | A Cranelift frame probes the stack | active |
| §56 | R26 — integer literals read at the target's width | active |
| §57 | R27 — field initializers run on every construction | active |
| §58 | R29 — a class index signature is accessor sugar | active |
| §59 | R30 — host-called entries take handle and scalar parameters | active |
| §60 | R31 — `using` declarations: deterministic scope-exit dispose | active |
| §61 | R32 — a wire-mapped alias in an entry signature | active |
| §62 | R33 — an alignment override on `@CStruct` value classes | active |
| §63 | R35 — a discovery check for one unresolved import | active |
| §64 | R36 — async methods on generic classes, generic async functions | active |
| §65 | R37 — a named accessor is method sugar | active |
| §66 | Emitted C identifiers — two spaces, never one | active |
| §67 | Checker scoping, and state that must survive a suspension | active |
| §68 | One ordered IR between the checker and the two tiers | active |
| §69 | The language definition, checked instead of asserted | active |
| §70 | A held async handle, by reference count | active |
| §71 | Static members | active |
| §72 | One integer-literal reader, and one HIR walk | active |
| §73 | The LIR terminator walks itself | active |
| §74 | One handle-kind table | active |
| §75 | Four LIR facts the form carries once | active |
| §76 | Three checker facts the HIR carries once | active |
| §77 | Two runtime facts written once | active |
| §78 | The MINOR consolidation pass | active |
| §79 | A divergence diagnostic shows the TypeScript form and the subscript form | active |
| §80 | Array data past `len` is zero | active |
| §81 | R38 — a write through a `@CStruct` copy that nothing reads | active |
| §82 | R39 — six requests decided at `e1c2be1` | active |
| §83 | The operation-signature table is a total function of the HIR | active |
| §84 | Worker messages carry `string` fields by copy | active |
| §85 | One gate command, two shapes | active |
...
Resolved 2026-07-26; the found state, the evidence, and what landed
are in `compiler-history.md` §19 (moved 2026-09-06). The current
rules it produced live in §20 (trap-site IR), §18 (the trap
observer), and `specs/tracking/p19-trap-parity.md`.
