...
fn jit_ship_c_aot_and_golden_agree_byte_for_byte() {
    ...
    assert_eq!(
        golden_ids.len(),
        153,
        "expected exactly 153 committed goldens: the 81 standing goldens (a01–a24 run set + a25–a39 interop \
         ...
         switch-body-scope goldens, the a149 §67 suspension-state golden, and the a150–a153 §68 \
         goldens, found {}",
        golden_ids.len()
    );
