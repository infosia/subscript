...
fn jit_ship_c_aot_and_golden_agree_byte_for_byte() {
    ...
    // a138–a139 pin R31 synchronous scope-exit disposal. Entry a140 pins
    ...
    // Entry a145 pins §66 emitted identifier spaces.
    // Entry a146 pins §66 managed scopes and lambda capture names.
    ...
    // Entry a159 pins a temporary base retained by a derived address.
    // Entry a160 pins module-initializer declaration order.
    // Entries a161–a163 pin counted async stores, the shared copy sites,
    // and address-taken activation liveness. Entries a164–a165 and a167 pin
    // §68 consumer decisions. Entry a166 pins resume-parameter interference
    ...
    // a178 pins generic instance and static method instances.
    // a179 pins a static read accessor without a write accessor.
    assert_eq!(
        golden_ids.len(),
        178,
        "expected exactly 178 committed goldens: the 81 standing goldens (a01–a24 run set + a25–a39 interop \
