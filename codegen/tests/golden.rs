...
fn jit_ship_c_aot_and_golden_agree_byte_for_byte() {
    ...
    // async capstone) plus the P9.1 stdlib entries (a40 Math battery,
    // a41 Math.random sequence).
    assert!(
        golden_ids.len() >= 41,
        "expected at least the 41 committed goldens (a01–a24 run set + a25–a39 interop \
         + a40–a41 stdlib), found {}",
        golden_ids.len()
    );
