...
fn jit_ship_c_aot_and_golden_agree_byte_for_byte() {
    ...
    // recursion review pin (a114), R14 Q32 alias switches (a115), and R15
    // divergence flow (a116).
    assert_eq!(
        golden_ids.len(),
        116,
        "expected exactly 116 committed goldens: the 81 standing goldens (a01–a24 run set + a25–a39 interop \
         ...
         alias-switch golden, and the a116 R15 divergence-flow golden, found {}",
        golden_ids.len()
    );
