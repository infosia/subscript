...
fn jit_ship_c_aot_and_golden_agree_byte_for_byte() {
    ...
    // Q26 Number formatting/clz32 (a62), and all five Q27 stages:
    // Math/Number, String, Array, Map/Set, and callback indices
    // (a63–a67).
    assert!(
        golden_ids.len() >= 67,
        "expected at least the 67 committed goldens (a01–a24 run set + a25–a39 interop \
         + a40–a45 stdlib + a46–a50 narrow numerics + a51–a56 Map/Set \
         + a57–a59 Number + a60 Unicode String + a61 SameValueZero \
         + a62 Q26 Number formatting/clz32 + a63–a67 Q27 stages 1–5), found {}",
        golden_ids.len()
    );
