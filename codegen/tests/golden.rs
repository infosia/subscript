...
fn jit_ship_c_aot_and_golden_agree_byte_for_byte() {
    ...
    assert_eq!(
        golden_ids.len(),
        158,
        "expected exactly 158 committed goldens: the 81 standing goldens (a01–a24 run set + a25–a39 interop \
         ...
         goldens, the a154–a155 §70 held-async-handle goldens, the a156 value-class receiver-by-value golden, the a157 suspension-loop-liveness golden, and the a159 address-base-liveness golden, found {}",
        golden_ids.len()
    );
