...
fn full_fails_on_release_skip() {
    ...
    // §85: the record names the release step's own environment.
...
fn moved_goldens_are_listed_without_failure() {
    ...
    let record = assert_record(
        &output,
        "full",
        "gate full 0123456789abcdef0123456789abcdef01234567 dirty:7 debug 5/0/3 release 5/0/3 skips 0/0 clippy 7/18/13 goldens-moved 6 exit 0",
    );
